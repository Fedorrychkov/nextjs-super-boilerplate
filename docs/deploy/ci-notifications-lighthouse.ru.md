...
| `check-agent-gate` | Гейт `guard-external.sh` без бита x, хук в `.claude/settings.json` без обёртки `[ -x ]` или не навешен: код 126/127 для PreToolUse значит «не мешать», и агентский слой выключается молча |
