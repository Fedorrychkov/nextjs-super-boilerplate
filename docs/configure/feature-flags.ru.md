...
|-------------------|-----------|
| `console` | Коды в логах сервера (dev/stage). Регистрация по коду и восстановление работают — код берётся из лога; в проде доктор предупредит |
| `elastic` | Elastic Email API — единственный боевой транспорт; уведомления по email идут только в нём |
| `empty` | Почты нет. Регистрация по коду отвечает ошибкой, восстановление по почте недоступно |

Любое другое значение — опечатка: почта не уходит, `pnpm doctor` предупреждает, регистрация по
коду отвечает ошибкой вместо «код отправлен». Событие уведомления с единственным каналом email в
режиме без доставки всё равно создаётся — со статусом «пропущено: почта не настроена».
