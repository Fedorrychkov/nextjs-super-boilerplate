...
import { listAiReferralVisitsForPathname } from '@lib/services/ai-referrals-pathname-detail.service'
...
const handler = (request: NextRequest, authResult: AuthSuccessResult) =>
  apiErrorHandlerContainer(request)(async (response: typeof NextResponse) => {
    ...
    const limitParsed = rawLimit != null && rawLimit !== '' ? Number.parseInt(rawLimit, 10) : undefined
    const limit = Number.isFinite(limitParsed) && limitParsed! > 0 ? limitParsed : undefined

    await connectDB()

    const data = await listAiReferralVisitsForPathname({
      pathname,
      windowDays: days,
      limit,
