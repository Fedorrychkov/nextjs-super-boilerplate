...
import { buildContentSuggestMessages } from '@lib/services/llm/build-content-suggest-prompt'
...
import { parseContentSuggestJson } from '~/api/llm'
import { UserRole } from '~/api/user'
import { getServerTFromNextRequestAsync } from '~/lib/i18n/server'
import { Logger } from '~/utils/logger'

const logger = new Logger(['LlmContentSuggestRoute', '[api/v1/llm/content/suggest]'])
...
const handlerPost = (request: NextRequest, authResult: AuthSuccessResult) =>
  apiErrorHandlerContainer(
    ...
    const { system, user } = buildContentSuggestMessages({ article, revision })
    ...
    try {
      ...
      const suggest = parseContentSuggestJson(completion.content.trim())

      if (!suggest) {
        logger.warn('content suggest JSON parse failed', { preview: completion.content?.slice(0, 200) })
      ...
      if (completion.usage) {
        try {
          ...
          logger.error('content suggest usage persist failed', {
      ...
      logger.error('content suggest failed', { message })
