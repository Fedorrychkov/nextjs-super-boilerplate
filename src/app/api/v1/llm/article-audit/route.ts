...
const handler = (request: NextRequest, authResult: AuthSuccessResult) =>
  apiErrorHandlerContainer(
    ...
    const { system, user } = buildArticleAuditMessages({ article, revision })
    ...
    try {
      ...
      const audit = parseArticleAuditJson(completion.content.trim())

      if (!audit) {
        logger.warn('article audit JSON parse failed', { preview: completion.content?.slice(0, 200) })
      ...
      logger.error('article audit failed', { message })
