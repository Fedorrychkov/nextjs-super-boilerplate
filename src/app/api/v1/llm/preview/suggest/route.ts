...
import { buildPreviewSuggestMessages } from '@lib/services/llm/build-preview-suggest-prompt'
...
import { parsePreviewSuggestJson } from '~/api/llm'
import { UserRole } from '~/api/user'
import { getServerTFromNextRequestAsync } from '~/lib/i18n/server'
import { Logger } from '~/utils/logger'

const logger = new Logger(['LlmPreviewSuggestRoute', '[api/v1/llm/preview/suggest]'])
...
const handlerPost = (request: NextRequest, authResult: AuthSuccessResult) =>
  apiErrorHandlerContainer(
    ...
    const { system, user } = buildPreviewSuggestMessages({ article, revision })
    ...
    try {
      ...
      const suggest = parsePreviewSuggestJson(completion.content.trim())

      if (!suggest) {
        logger.warn('preview suggest JSON parse failed', { preview: completion.content?.slice(0, 200) })
      ...
      if (completion.usage) {
        try {
          ...
          logger.error('preview suggest usage persist failed', {
      ...
      logger.error('preview suggest failed', { message })
