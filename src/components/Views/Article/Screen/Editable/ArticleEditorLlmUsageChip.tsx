...
export const ArticleEditorLlmUsageChip = (props: Props) => {
  ...
  const { chatTokens, auditTokens, structuredTokens, imageTokens, ttsTokens } = useMemo(() => {
    ...
    const structured =
      (by.find((x) => x.source === 'seo_suggest')?.totalTokens ?? 0) +
      (by.find((x) => x.source === 'preview_suggest')?.totalTokens ?? 0) +
      (by.find((x) => x.source === 'content_suggest')?.totalTokens ?? 0)
    const image =
      (by.find((x) => x.source === 'image_generate')?.totalTokens ?? 0) +
      (by.find((x) => x.source === 'image_prompt_stream')?.totalTokens ?? 0) +
      (by.find((x) => x.source === 'image_prompt_article')?.totalTokens ?? 0)
    const tts = by.find((x) => x.source === 'listen_tts')?.totalTokens ?? 0

    return { chatTokens: chat, auditTokens: audit, structuredTokens: structured, imageTokens: image, ttsTokens: tts }
  ...
  return (
    <Typography variant="Body/XS/Regular" className={cn('max-w-[min(100%,480px)] text-muted-foreground')}>
      {t('article.ui.aiEditorUsageSummary', {
        ...
        image: imageTokens.toLocaleString(),
        tts: ttsTokens.toLocaleString(),
      })}
    </Typography>
  )
}
