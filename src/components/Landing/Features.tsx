      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="text-center mb-12">
          <Typography asTag="h2" variant="heading-2" className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground">
            {t('nbs.features.title')}
          </Typography>
          <Typography className="mt-3 text-muted-foreground max-w-xl mx-auto">{t('nbs.features.subtitle')}</Typography>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {LANDING_FEATURE_IDS.map((id) => (
            <div key={id} className="group rounded-2xl border border-border bg-card p-5 hover:border-border/80 hover:shadow-sm transition-all">
              ...
              <Typography asTag="h3" variant="heading-3" className="font-semibold text-foreground text-sm mb-1">
                {t(`nbs.features.items.${id}.title`)}
              </Typography>
              <Typography className="text-xs text-muted-foreground leading-relaxed">{t(`nbs.features.items.${id}.description`)}</Typography>
            </div>
          ))}
        </div>
      </div>
